...
[26/06/2026] Trước khi connect 1 platform mới vào pipeline đăng tự động nhiều platform (`scripts/social/post-all.js`), BẮT BUỘC audit giới hạn nội dung riêng (caption length, media spec, rate limit) của platform đó — KHÔNG giả định platform mới dùng chung constraint với platform cũ. Nếu giới hạn khác biệt lớn (vd Threads 500 ký tự vs Facebook ~63K), phải xử lý riêng (bản nội dung rút gọn/khác cấu trúc) và coi platform đó "best-effort" (lỗi không chặn advance queue của platform khác) cho đến khi verify ổn định — Xác nhận qua sự cố thực tế (postmortem `docs/postmortems/2026-06-26-threads-caption-limit-duplicate-posts.md`: Threads vượt 500 ký tự khiến 1 bài bị đăng trùng 13 lần lên Facebook+Instagram vì code coi 3 platform đồng hạng)
